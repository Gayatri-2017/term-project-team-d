...
install-prom:
	...
	$(HELM) install $(RELEASE) -f $(YAML_DIR)/helm-kube-stack-values.yaml --namespace $(ISTIO_NS) prometheus-community/kube-prometheus-stack | tee -a $(LOG_DIR)/obs-install-prometheus.log
	$(KC) apply -n $(ISTIO_NS) -f $(YAML_DIR)/monitoring-lb-services.yaml | tee -a $(LOG_DIR)/obs-install-prometheus.log
	$(KC) apply -n $(ISTIO_NS) -f $(YAML_DIR)/grafana-flask-configmap.yaml | tee -a $(LOG_DIR)/obs-install-prometheus.log
